...
const ERROR_MESSAGE_NO_LABEL = 'elements require a valid associated label.';
const ERROR_MESSAGE_MULTIPLE_LABEL = 'elements should not have multiple valid labels.';
  ...
  visitor() {
    return {
      ...
      MustacheStatement(node, path) {
        ...
        this.log({
          message: `${node.path.original} ${ERROR_MESSAGE_NO_LABEL}`,
          line: node.loc && node.loc.start.line,
          column: node.loc && node.loc.start.column,
          source: this.sourceForNode(node),
        });
